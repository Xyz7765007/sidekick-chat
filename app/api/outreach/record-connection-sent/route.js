// PROXY: /api/outreach/record-connection-sent → SignalScope /api/outreach
// action: record_manual_connection_sent (MANUAL-WITH-ASSIST — no Unipile send).
// Records that the exec sent the connection request by hand on LinkedIn.
...
export async function POST(request) {
  ...
  try {
    ...
    const r = await fetch(upstream, {
      ...
      body: JSON.stringify({ ...body, action: "record_manual_connection_sent", baseId: BASE_ID }),
      cache: "no-store",
    });
