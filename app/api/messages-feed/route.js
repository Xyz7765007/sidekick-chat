...
export async function GET(req) {
  ...
  const limit = inUrl.searchParams.get("limit") || "20";
  try {
    const upstream = `${SIGNALSCOPE_URL.replace(/\/$/, "")}/api/sidekick/messages-feed?baseId=${encodeURIComponent(BASE_ID)}&limit=${limit}`;
